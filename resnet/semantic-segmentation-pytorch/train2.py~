...
from dice_coeff_loss import dice_coeff_loss
...
def train(segmentation_module, iterator, optimizers, history, epoch, args):
    ...
    for i in range(args.epoch_iters):
        ...
        ave_iou.update(iou.data.item())
        
        # calculate accuracy, and display
        if i % args.disp_iter == 0:
            print('Epoch: [{}][{}/{}], Time: {:.2f}, Data: {:.2f}, '
                  'lr_encoder: {:.6f}, lr_decoder: {:.6f}, '
                  'Accuracy: {:4.2f}, IOU: {:.6f} Loss: {:.6f}'
                  .format(epoch, i, args.epoch_iters,
                          batch_time.average(), data_time.average(),
                          args.running_lr_encoder, args.running_lr_decoder,
                          ave_acc.average(), ave_iou.average(), ave_total_loss.average()))

            fractional_epoch = epoch - 1 + 1. * i / args.epoch_iters
            history['train']['epoch'].append(fractional_epoch)
            history['train']['loss'].append(loss.data.item())
            history['train']['acc'].append(acc.data.item())
            history['train']['iou'].append(iou.data.item())
...
def main(args):
    ...
    crit = nn.NLLLoss(weight=torch.tensor([0.00052, 0.99948]), ignore_index=-100)
    #crit = dice_coeff_loss
